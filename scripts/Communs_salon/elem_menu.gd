...
func _on_mur_color_selected(id: int) -> void:
	...
	var beige = get_parent().get_node("murs/beigeWall")
	var rouge = get_parent().get_node("murs/redWall")
	var vert = get_parent().get_node("murs/greenWall")

	var num = id 
	match num:
		0:
			murs.change_couleur(beige)
		1 :
